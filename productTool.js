var editorTemplate = '<button id="addProduct" class="button">Add Product</button>';
...
var toolTemplate = function (values, isViewer = false) {
  return `<div class="product-card" style="position:relative;display:table;min-width:0;word-wrap:break-word;background-color:#fff;background-clip:border-box;border:1px solid rgba(0,0,0,.125);border-radius:4px;margin:auto;text-align:center;">
    <img src="${values.productImage.url}" style="width: 100%; object-fit: contain; border-top-left-radius: 4px; border-top-right-radius: 4px;" />
    <div class="product-card-body" style="padding: 0 16px 16px;text-align: left;">
      <h3 style="margin: 12px 0; color: ${values.productTitleColor};">${values.productTitle}</h3>
    ...
    <div class="product-footer" style="display: flex; border-bottom-left-radius: 4px; border-bottom-right-radius: 4px; border-top: 1px solid rgba(0,0,0,.125); align-items: center; font-weight: bold; background-color: ${values.productPriceBackgroundColor};">
      <div style="width: 50%; padding: 12px; font-size: 16px; line-height: 1.5; border-bottom-left-radius: 4px; color: ${values.productPriceColor};">$${values.productPrice}</div>
      <a class="button no-underline no-border-radius" href="${values.productCTAAction.url}" target="${values.productCTAAction.target}" style="width: 50%; text-decoration: none; border-bottom-right-radius: 4px; background-color: ${values.productCTAColor}; color: ${values.productCTATextColor}; display: inline-block; font-weight: 400; text-align: center; vertical-align: middle; border: 1px solid transparent; padding: 12px; font-size: 16px; line-height: 1.5; transition: color .15s ease-in-out,background-color .15s ease-in-out,border-color .15s ease-in-out,box-shadow .15s ease-in-out; cursor: pointer;">${values.productCTA}</a>
    </div>
  </div>
      ...
      addButton.onclick = function () {
        ...
        setTimeout(() => {
          ...
          selectButton.onclick = function (e) {
            if (e.target.id === 'product-item') { // If user clicks on product item
              // Find selected item from products list
              const selectedProduct = data.products.find(item => item.id === parseInt(e.target.dataset.uuid));
              updateValue({ selected: selectedProduct });
            } else { // If user click on child of product item (e.g. title, price, image or desctiption)
              const parent = e.target.parentElement;
              if (parent && parent.id !== 'product-item') return;
              const selectedProduct = data.products.find(item => item.id === parseInt(parent.dataset.uuid));
              updateValue({ selected: selectedProduct });
            }
            hideModal();
          ...
          searchButton.onclick = function (e) {
            ...
            if (list && data && data.products) {
              if (searchBar.value === "") {
                ...
                filteredItem = data.products.filter(item => item.title.toLowerCase().includes(searchBar.value.toLowerCase()));
