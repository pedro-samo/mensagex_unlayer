      ...
      let newValues =
          name === "productLibrary"
              ? {
                    ...
                    produtctOldPrice: value.selected.oldPrice ? value.selected.oldPrice.toFixed(2).toString().replace('.', ',') : '',
                    productPrice: value.selected.price.toFixed(2).toString().replace('.', ','),
...
const getProduct = async (title, userToken) => {
  ...
  const replaceDot = (productsArray) => {
    if(!productsArray.length) return
    const newProductsArray = productsArray.map(product => {
      const newPrice = product.price.toFixed(2).toString().replace('.', ',');
      const newOldPrice = product.oldPrice ? product.oldPrice.toFixed(2).toString().replace('.', ',') : '';
      return {...product, price: newPrice, oldPrice: newOldPrice}
    })
  ...
  try {
      const url = `https://mensagex.com.br/s/ecomm/products/search?title=${removeSpecialCharacters(title)}&token=${userToken}`;
      const response = await fetch(url);
      const data = await response.json();
      productList = data;
      showApiResponse(replaceDot(data), title);
  } catch (e) {
